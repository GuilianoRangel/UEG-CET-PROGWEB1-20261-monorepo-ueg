...
- [x] Criar interface `MatchDto` e `PredictionOptionDto`.
- [x] Criar `CreateMatchPayload` contendo os dados do jogo + `options: Array<{ teamAScore: number, teamBScore: number }>`.
- [x] Garantir exportação no `packages/utils/src/index.ts`.

## Fase 2: Backend - Módulo de Jogos (`MatchesModule`)
> **Onde:** `apps/backend/src/matches/`

**Detalhamento Técnico:**
- [x] **DTOs (`create-match.dto.ts`):** 
...
- [x] 1. `should create a match with valid data and up to 4 options`: Testa caminho feliz.
- [x] 2. `should throw BusinessException if options length is greater than 4`: Força erro validando a restrição principal.
- [x] 3. `should close a match manually`: Garante que a flag `isClosed` é alternada.
- [x] 4. `should identify match as closed dynamically`: Uma função helper `isMatchClosed(match)` deve retornar `true` se `matchDate < new Date()`.

## Fase 3: Backend - Módulo de Palpites (`GuessesModule`)
> **Onde:** `apps/backend/src/guesses/`

**Detalhamento Técnico:**
- [x] **Endpoints (Usuário Comum):**
...
- [x] 1. `should create a user guess successfully for an open match`: Testa inserção normal.
- [x] 2. `should throw BusinessException if user tries to guess twice on the same match`: Regra de unicidade.
- [x] 3. `should throw BusinessException if user tries to guess on a closed match`: Validar `isMatchClosed` antes de salvar. Impede palpites tardios.
- [x] 4. `should return guessCounts correctly mapped to predictionOptions`: Verifica se o retorno de contagem social está acurado.
...
- [x] **Serviço HTTP:** `MatchesService` implementando as chamadas para o backend (`POST /matches`, etc).
- [x] **Lista (`matches-list.component`):** Usar o design system. Implementar pipe/função para formatar o status ("Aberto" / "Encerrado"). Ação de exclusão abre modal customizado.
- [x] **Formulário Reativo (`matches-form.component`):**
...
- [x] **Painel (`guess-board.component`):**
